  ...
  if (keychain && layerConfig.length > 0) {
    ...
    if (softenOuter3mf) {
      if (isRight3mf) {
        ...
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge, outerR), 0°→90° CCW — rises above outerR
        ...
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge, -outerR), 270°→360° CCW — dips below −outerR
          const a = -Math.PI / 2 + (Math.PI / 2) * i / Nfo3mf;
          outerDPath.push(toClip3mf(badgeEdge3mf + ofr3mf * Math.cos(a), -outerR + ofr3mf * Math.sin(a)));
        }
      } else {
        outerDPath.push(toClip3mf(badgeEdge3mf - ofr3mf, -outerR));
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge, -outerR), 180°→270° CCW — dips below −outerR
        ...
        outerDPath.push(toClip3mf(extendX,  outerR + ofr3mf));
        outerDPath.push(toClip3mf(badgeEdge3mf, outerR + ofr3mf));
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge, outerR), 90°→180° CCW — rises above outerR
          const a = Math.PI / 2 + (Math.PI / 2) * i / Nfo3mf;
          outerDPath.push(toClip3mf(badgeEdge3mf + ofr3mf * Math.cos(a), outerR + ofr3mf * Math.sin(a)));
        }
      }
    ...
    if (diffResult.length) {
      ...
      let ringGeo = _badgeBuildSolidExtrusionMesh(ringOuters, zOff, offX, offY);
      ringGeo = _badgeMergeVerticesForExport(ringGeo);
      objects.push({ geo: ringGeo, name: 'Keychain_Ring', colour: redLayer.hex, extruder: 1, id: objects.length + 1, skipFilamentSlot: true });

      // Inner D-shape as negative part — Bambu Studio subtracts to cut the keyhole
      const innerForNeg = ClipperLib.Clipper.Orientation(innerDPath) ? [innerDPath] : [[...innerDPath].reverse()];
      let holeGeo = _badgeBuildSolidExtrusionMesh(innerForNeg, zOff, offX, offY);
