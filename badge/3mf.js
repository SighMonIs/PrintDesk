  ...
  if (keychain && layerConfig.length > 0) {
    ...
    if (softenOuter3mf) {
      if (isRight3mf) {
        ...
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge+ofr, outerR-ofr), 90°→180° CCW
        ...
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge+ofr, -outerR+ofr), 180°→270° CCW
          const a = Math.PI + (Math.PI / 2) * i / Nfo3mf;
          outerDPath.push(toClip3mf(badgeEdge3mf + ofr3mf + ofr3mf * Math.cos(a), -outerR + ofr3mf + ofr3mf * Math.sin(a)));
        }
      } else {
        outerDPath.push(toClip3mf(badgeEdge3mf - ofr3mf, -outerR));
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge-ofr, -outerR+ofr), 270°→360° CCW
          const a = -Math.PI / 2 + (Math.PI / 2) * i / Nfo3mf;
          outerDPath.push(toClip3mf(badgeEdge3mf - ofr3mf + ofr3mf * Math.cos(a), -outerR + ofr3mf + ofr3mf * Math.sin(a)));
        }
        outerDPath.push(toClip3mf(extendX, -outerR + ofr3mf));
        outerDPath.push(toClip3mf(extendX,  outerR - ofr3mf));
        outerDPath.push(toClip3mf(badgeEdge3mf, outerR - ofr3mf));
        for (let i = 0; i <= Nfo3mf; i++) { // centre (badgeEdge-ofr, outerR-ofr), 0°→90° CCW
