// Fallback if config.js missing
window.CONFIG = window.CONFIG || { SUPABASE_URL: '', SUPABASE_KEY: '' };
</script>
</head>
<body>
<div class="app">
  ...
  <div class="setup-banner" id="setupBanner" style="display:none">
    <h3><i class="ti ti-plug" style="vertical-align:-2px;margin-right:6px"></i>Connect to Supabase</h3>
    <p>Enter your Supabase project URL and anon key to connect PrintDesk to your database.</p>
    <div class="url-input-row" style="margin-bottom:6px">
      <input type="url" id="gasUrlInput" placeholder="https://xxxxxxxxxxxx.supabase.co">
    </div>
    <div class="url-input-row">
      <input type="text" id="sbKeyInput" placeholder="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9…">
      <button class="btn primary" onclick="saveSupabaseConfig()"><i class="ti ti-check"></i> Connect</button>
    </div>
  </div>

  <!-- Header -->
  <div class="hdr">
    <div><span class="logo">PrintDesk</span></div>
    <div class="hdr-actions">
      <button class="btn" onclick="openColourModal()"><i class="ti ti-brush"></i><span class="btn-label"> Colours</span></button>
      <div class="btn-group"><button class="btn" onclick="openCatModal()"><i class="ti ti-category"></i><span class="btn-label"> Categories</span></button><button class="btn" onclick="openOptModal()"><i class="ti ti-list-details"></i><span class="btn-label"> Options</span></button></div>
      <button class="btn" onclick="loadAll()"><i class="ti ti-refresh"></i><span class="btn-label"> Refresh</span></button>
      <button class="btn primary" onclick="openAddModal()"><i class="ti ti-plus"></i><span class="btn-label"> New Order</span></button>
      <button class="btn icon-only" onclick="openStatsModal()" title="Stats"><i class="ti ti-chart-bar"></i></button>
      <button class="btn icon-only" onclick="openSettings()" title="Settings"><i class="ti ti-settings"></i></button>
    </div>
  </div>

  <!-- Hidden stat elements still needed for updateStats() -->
  <span id="s-total" style="display:none"></span>
  <span id="s-pending" style="display:none"></span>
  <span id="s-printing" style="display:none"></span>
  <span id="s-completed" style="display:none"></span>
  <span id="s-revenue" style="display:none"></span>
  <span id="s-cat-breakdown" style="display:none"></span>
  <span id="s-badge-breakdown" style="display:none"></span>
  <span id="s-payment-breakdown" style="display:none"></span>

  <!-- Toolbar -->
  <div class="toolbar">
    <input type="text" id="search" placeholder="Search customer, model, address, notes…" oninput="renderTable()">
    <select id="filterStatus" onchange="renderTable()">
      <option value="">All statuses</option>
      <option>Pending</option><option>Printing</option><option>Complete</option><option>Cancelled</option>
    </select>
    <select id="filterCat" onchange="renderTable()"><option value="">All categories</option></select>
    <label class="toolbar-check"><input type="checkbox" id="hideCompleted" onchange="renderTable()"> Hide Completed</label>
  </div>
  ...
  <div class="tbl-wrap">
    <table>
      <thead>
        <tr>
          <th class="sortable" style="width:44px" data-key="orderId" onclick="sortBy('orderId')"># <span class="sort-arrow" data-key="orderId"></span></th>
          <th class="sortable" style="width:130px" data-key="customer" onclick="sortBy('customer')">Customer <span class="sort-arrow" data-key="customer"></span></th>
          <th class="sortable" style="width:170px" data-key="address" onclick="sortBy('address')">Address <span class="sort-arrow" data-key="address"></span></th>
          <th class="sortable" style="width:150px" data-key="catId" onclick="sortBy('catId')">Category <span class="sort-arrow" data-key="catId"></span></th>
          <th style="width:160px">Options</th>
          <th class="sortable" style="width:44px" data-key="qty" onclick="sortBy('qty')">Qty <span class="sort-arrow" data-key="qty"></span></th>
          <th class="sortable" style="width:80px" data-key="total" onclick="sortBy('total')">Total <span class="sort-arrow" data-key="total"></span></th>
          <th class="sortable" style="width:130px" data-key="status" onclick="sortBy('status')">Status <span class="sort-arrow" data-key="status"></span></th>
...
<div class="statusbar"><div class="dot" id="dot"></div><span id="statusMsg">Loading…</span><span style="margin-left:auto;font-family:'DM Mono',monospace;font-size:11px;color:var(--border2)">v2.1.6</span></div>

<!-- ══ Order Modal ══ -->
<div class="modal-bg" id="orderModal" onclick="if(event.target===this)closeModal()">
  <div class="modal" onclick="event.stopPropagation()">
    ...
    <div class="field-row">
      <div class="field"><label>Customer name</label><input id="f-customer" type="text" placeholder="Jane Smith" oninput="this.closest('.field').classList.remove('field-error');this.closest('.field').querySelector('.field-error-msg')?.remove()"></div>
      <div class="field"><label>Delivery type</label>
        <select id="f-delivery"><option value="Post">Post</option><option value="Pick Up">Pick Up</option></select>
      </div>
    </div>
    <div class="field-row">
      <div class="field">
        ...
        <div class="addr-wrap">
          <input id="f-address" type="text" placeholder="Address or location note…" autocomplete="off" oninput="this.closest('.field').classList.remove('field-error');this.closest('.field').querySelector('.field-error-msg')?.remove()">
      ...
      <div class="field"><label>Payment</label>
        <select id="f-payment"><option value="No">No</option><option value="Free">Free</option><option value="Simon">Simon</option><option value="Wade">Wade</option></select>
      </div>
    </div>
    ...
    <div class="modal-actions">
      <div class="order-total-inline">
        ...
        <span class="order-total-val" id="orderTotal">$0.00</span>
      </div>
...
<div class="modal-bg" id="noteModal" onclick="if(event.target===this)closeNoteModal()">
  <div class="modal sm" onclick="event.stopPropagation()">
    ...
    <div class="note-view" id="noteModalBody"></div>
    <div class="modal-actions"><button class="btn primary" onclick="closeNoteModal()">Close</button></div>
  </div>
</div>
...
<div class="modal-bg" id="catModal" onclick="if(event.target===this)closeCatModal()">
  <div class="modal md" onclick="event.stopPropagation()">
    ...
    <p style="font-size:12px;color:var(--muted);margin-bottom:1rem;line-height:1.7">Simple list of categories, each with a name and default price.</p>
    <div class="cat-flat-hdr"><span>ID</span><span>Name</span><span>Price ($)</span><span></span></div>
    <div class="cat-flat-list" id="catFlatList"></div>
    <button class="btn" style="width:100%" onclick="addCat()"><i class="ti ti-plus"></i> Add category</button>
    <div class="modal-actions">
      <button class="btn" onclick="closeCatModal()">Cancel</button>
      <button class="btn primary" onclick="saveCats()"><i class="ti ti-cloud-upload"></i> Save</button>
    </div>
  </div>
</div>

<!-- ══ Options Modal ══ -->
<div class="modal-bg" id="optModal" onclick="if(event.target===this)closeOptModal()">
  <div class="modal" onclick="event.stopPropagation()">
    <div class="modal-title">Manage options</div>
    <p style="font-size:12px;color:var(--muted);margin-bottom:1rem;line-height:1.7">Options are extra fields shown when adding a model of a given category. Use <strong>text</strong> for a free-text field, <strong>dropdown</strong> for a pick list (comma-separated values). Adding "Custom" to a dropdown shows a free-text box when selected.</p>
    <div class="opt-hdr"><span>ID</span><span>Category</span><span>Field name</span><span>Display</span><span></span></div>
    <div class="opt-list" id="optList"></div>
    <button class="btn" style="width:100%" onclick="addOpt()"><i class="ti ti-plus"></i> Add option</button>
    <div class="modal-actions">
      <button class="btn" onclick="closeOptModal()">Cancel</button>
      <button class="btn primary" onclick="saveOpts()"><i class="ti ti-cloud-upload"></i> Save</button>
    </div>
  </div>
</div>
...
<!-- ══ Colours Modal ══ -->
<div class="modal-bg" id="colourModal" onclick="if(event.target===this)closeColourModal()">
  <div class="modal md" onclick="event.stopPropagation()">
    <div class="modal-title"><i class="ti ti-brush" style="margin-right:8px;vertical-align:-2px"></i>Colours</div>
    <p style="font-size:12px;color:var(--muted);margin-bottom:1rem;line-height:1.7">Manage your available filament colours. Tick <strong>Available</strong> if you currently have that colour in stock.</p>
    <div class="colour-mgr-hdr">
      <span>Swatch</span><span>Name</span><span>Hex code</span><span>Available</span><span></span>
    </div>
    ...
    <div class="modal-actions">
      <button class="btn" onclick="closeColourModal()">Cancel</button>
...
<!-- ══ Stats Modal ══ -->
<div class="modal-bg" id="statsModal" onclick="if(event.target===this)closeStatsModal()">
  <div class="modal md" onclick="event.stopPropagation()">
    <div class="modal-title"><i class="ti ti-chart-bar" style="margin-right:8px;vertical-align:-2px"></i>Stats</div>
    <div class="stats-grid" id="statsGrid"></div>
    <div class="modal-actions">
      <button class="btn primary" onclick="closeStatsModal()">Close</button>
    </div>
  </div>
</div>
...
<div class="modal-bg" id="settingsModal" onclick="if(event.target===this)closeSettings()">
  <div class="modal md" onclick="event.stopPropagation()">
    <div class="modal-title"><i class="ti ti-settings" style="margin-right:8px;vertical-align:-2px"></i>Settings</div>
    <div class="settings-section">
      <div class="settings-section-title">Supabase Connection</div>
      <div class="field">
        <label>Supabase Project URL</label>
        <input type="url" id="settingsGasUrl" placeholder="https://xxxxxxxxxxxx.supabase.co">
        <div class="field-hint">Your Supabase project URL.</div>
      </div>
    </div>
    <div class="settings-section">
      <div class="settings-section-title">Google Sheet</div>
      <div class="field"><label>Google Sheet URL</label>
        <input type="url" id="settingsSheetUrl" placeholder="https://docs.google.com/spreadsheets/d/…">
      </div>
      <a id="sheetLink" href="#" target="_blank" class="settings-link" style="opacity:0.4"><i class="ti ti-table"></i> Open Google Sheet</a>
    </div>
    <div class="settings-section">
      <div class="settings-section-title">Accent colour</div>
      <div class="colour-swatches" id="swatchRow"></div>
      <div class="colour-custom">
        <input type="color" id="customColour" oninput="previewAccent(this.value)">
        <label>Custom colour</label>
      </div>
    </div>
    <div class="modal-actions">
      <button class="btn" onclick="closeSettings()">Cancel</button>
      <button class="btn primary" onclick="applySettings()"><i class="ti ti-check"></i> Save settings</button>
    </div>
  </div>
</div>
...
const SB_HEADERS = () => ({
  ...
  'Authorization': 'Bearer ' + getCfg('SUPABASE_KEY'),
...
function applyAccent(a,a2,save=true){document.documentElement.style.setProperty('--accent',a);document.documentElement.style.setProperty('--accent2',a2);if(save)localStorage.setItem('pd_accent',JSON.stringify({a,a2}));}
